...
const Update = () => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    var { deviceName, deviceType } = document.forms[0];
    const access_token = localStorage.getItem("access_token") || "";
    // TODO: change endpoint default
    let config = {
      method: "put",
      url: `http://localhost:8080/device/${params.id}`,
      headers: {
  ...
  useEffect(() => {
    ...
    function handleCheckboxChange(event: any): void {
        var { deviceName, deviceType } = document.forms[0];
        setDevice({
            deviceName: deviceName.value,
            deviceType: deviceType.value,
            ...
            <div className="flex-auto px-4 lg:px-10 py-10 pt-0">
              <form onSubmit={handleSubmit}>
                ...
                <div className="flex gap-3 max-w-sm">
                  <button
                    className="py-2.5 px-6 rounded-lg text-sm font-medium bg-primary-200 text-teal-800"
